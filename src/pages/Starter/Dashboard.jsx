  ...
  return (
    <div className="min-h-screen bg-white px-4 sm:px-8 py-6">
      ...
      <div className="max-w-7xl mx-auto border rounded-xl shadow-md p-6 sm:p-8 mt-6">
        <div className="divider text-lg font-semibold text-gray-800">
          รถกอล์ฟ
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {cartStatus.map((status, idx) => (
            <StatusCard key={idx} image={cartImage} {...status} className="" />
          ))}
        </div>

        <div className="divider text-lg font-semibold text-gray-800 mt-6">
          กระเป๋าไม้กอล์ฟ
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {bagStatus.map((status, idx) => (
            <StatusCard key={idx} image={bagImage} {...status} className="" />
          ))}
        </div>
      </div>
    </div>
